...
def doctor_dashboard(request):
    ...
    pend = Appointment.objects.filter(doctor=Doctor.objects.get(user=request.user),status="awaiting")
    ...
    t_today = today.count()
    t_awaiting = pend.count()
    d = {'data': data, 'total': c, 'up': up, 'today': today,'t_today':t_today,'t_awaiting':t_awaiting}
...
def hospital_dashboard(request):
    ...
    pend=Hospital_Appointment.objects.filter(hospital=Hospital.objects.get(user=request.user),status="awaiting")
...
def Registeration(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if m == "Doctor":
                Doctor.objects.create(user=user,status="awaiting")
            if m == "Hospital":
                Hospital.objects.create(user=user,status="awaiting")
            if m == "Medical":
                Medical.objects.create(user=user,status="awaiting")
...
def appointment(request,pid):
    ...
    if request.method == "POST":
        ...
        app=Appointment.objects.create(doctor=doctor,patient=Patient.objects.get(user=request.user),a_date=a,status="awaiting",p_status="awaiting")
...
def p_appointment(request):
    data=Appointment.objects.filter(patient=Patient.objects.get(user=request.user),status="awaiting")
    d={'data':data}
...
def d_appointment(request):
    if not access(request.user):
        ...
        return redirect('doctor_profile')
    data=Appointment.objects.filter(doctor=Doctor.objects.get(user=request.user),status="awaiting")
    d={'data':data}
...
def hospital_appointment(request,pid):
    ...
    if request.method == "POST":
        a = request.POST['a_date']
        app=Hospital_Appointment.objects.create(hospital=hospital,patient=Patient.objects.get(user=request.user),a_date=a,status="awaiting",p_status="awaiting")
        messages.success(request,"Appointment Request Sent Successfully")
...
def hospital_view_appintment(request):
    if not access(request.user):
        messages.success(request,'Update Your Profile and Wait for Verification')
        return redirect('hospital_profile')
    data=Hospital_Appointment.objects.filter(hospital=Hospital.objects.get(user=request.user),status="awaiting",)
    d={'data':data}
...
def h_appointment(request):
    data=Hospital_Appointment.objects.filter(patient=Patient.objects.get(user=request.user),status="awaiting")
    d={'data':data}
...
def delete_doctor(request, did):
    data = Doctor.objects.get(id=did)
    data.delete()
